...
const CoinsCounter = ({allCoins, currentCoinsCount,...props}: Props) => {
  return (
    <div className="absolute right-0 top-0 my-3 mx-4 flex items-center gap-1">
      <img src={coinPNG} alt="Монета" className="w-8 h-6 object-cover"/>
      <div className="rounded-lg w-32 h-6 relative overflow-hidden">
        <div className="bg-gradient-to-b from-zinc-200 from-30% to-zinc-400 to-100% h-full">
        </div>
        <div
        ...
        ></div>
        <div className="w-full h-full absolute top-0 left-0 flex justify-center items-center z-50 mix-blend-difference">
          <p className="text-slate-700 font-medium text-sm">{`${currentCoinsCount}/${allCoins}`}</p>
        </div>
